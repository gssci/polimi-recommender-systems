...
def similarity(u,v):
    SHRINK = 7
    a = np.dot(u,v)
    b = np.dot(np.norm(u),np.norm(v)) + SHRINK
    return a/b

ratings = pd.DataFrame(columns=('user_id', 'item_id', 'rating'))

j=0
simil_matrix = np.ndarray(shape=(users.shape[0],items.shape[0]))
for u in range(users.shape[0]):
    ...
    for i in range(items.shape[0]):
        ...
        item = items.ix[[i]].values
        simil_matrix[u][i] = 1 - d.cosine(user,item)
