...
alltitle = [titl for title in list(map(spit, corpus)) for titl in title]
fdist = nltk.FreqDist(alltitle)
title = [k for k in fdist.keys() if fdist.get(k) > 1]
title = np.array(title).astype(int)

data = pd.concat([pd.Series(row['id'], row['title'].split(',')) for _, row in items.iterrows()]).reset_index()
data = data[data.columns[::-1]].rename(index=str, columns={"index": "titl", 0: "item_id"})
data = data.reset_index().drop('index', 1)
data['titl'] = data['titl'].astype(int)
data = data[data.titl.isin(title)]

title = np.unique(data['titl'].values)
titl_to_index = dict(zip(title, range(len(title))))

data['y'] = 1
data['item_index'] = [ids_to_index.get(i) for i in data['item_id']]
data['titl_index'] = [titl_to_index.get(i) for i in data['titl']]

icm = sps.csr_matrix((data['y'], (data['item_index'], data['titl_index'])))
...
for u in users:
    ...
    remaining -= 1
    print(str(len(rec.nonzero()[0])) +" - "+str(remaining))

save_sparse_csr('CBF_title_ALL',sps.csr_matrix(R_out))